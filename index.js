...
const totalamtraised = GAMES_JSON.reduce((total, game)=>{return total + game.pledged}, 0); 


// set inner HTML using template literal
raisedCard.innerHTML = `$${totalamtraised.toLocaleString()}`;
...
function filterUnfundedOnly() {
    ...
    console.log("unfunded game ", unfundedGames);
...
function filterFundedOnly() {
    ...
    console.log("funded game ", fundedGames);
...
const displaystr = `A total of $${totalPledged.toLocaleString()} has been pledged for ${totalGames} ${totalGames === 1 ? 'game' : 'games'}.
  Currently, ${numOfUnfunded} ${numOfUnfunded === 1 ? 'game remain' : 'games remains'} unfunded. We need your help to fund these amazing games!` ;



// create a new DOM element containing the template string and append it to the description container
const descrp = document.createElement("p");
descrp.innerText = displaystr;
descriptionContainer.appendChild(descrp);
...
SecondTopGame.innerText = `${secondGame.name}`;
